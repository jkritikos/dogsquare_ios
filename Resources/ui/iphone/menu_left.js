...
leftTableView.addEventListener("click", function(e){
	...
	switch(menuItem){
		...
		case MENU_GALLERY:
			...
			buildGalleryView(userObject.userId, PHOTO_TYPE_USER);
...
function handleLeftSearchResultRows(e){
	...
	if (userId == userObject.userId){
		...
		var checkinPlaceView = buildCheckinPlaceView(placeId, false, TARGET_MODE_REUSE);
...
function updateLeftMenuCounts(cFollowers, cInbox, cNotifications){
	...
	//refresh user thumbnail if it's remote image
	if(userObject != null && userObject.thumb_path && userObject.thumb_path.substring(0,4) == 'http'){
		leftTableView.data[0].rows[0].children[0].image = getUserPhoto(userObject.thumb_path);
	}
